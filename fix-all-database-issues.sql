...
-- Comprehensive fix for all the failing tables and data
-- =====================================================
...
-- Step 2: Create missing user profiles for all auth users
DO $$
DECLARE
    auth_user RECORD;
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM public.user_profiles WHERE id = auth_user.id) THEN
            ...
            INSERT INTO public.user_profiles (
                ...
                auth_user.id,
                auth_user.email,
                COALESCE(auth_user.user_metadata->>'first_name', auth_user.user_metadata->>'name', 'User'),
                COALESCE(auth_user.user_metadata->>'last_name', ''),
                COALESCE(auth_user.user_metadata->>'full_name', auth_user.user_metadata->>'name', auth_user.email),
                NOW(),
                NOW()
            );
